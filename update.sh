#!/usr/bin/env bash
# Update crispz (Unix): recupere les commits GitHub puis remet les dependances
# en phase avec le lock, SANS casser l'installation existante.
#
...
if [ "$NEEDDEPS" = "0" ]; then
  ...
  # Pillow est pose a part, en --no-deps: la borne pillow<12 de gradio rendrait
  # la resolution impossible. Cf. install.sh.
...
"$RUNPY" -c "from diffusers import ZImageImg2ImgPipeline; print('diffusers: ZImageImg2ImgPipeline OK')" || {
  echo "[ERREUR] diffusers ne fournit plus ZImageImg2ImgPipeline."
  echo "  Relance install.sh, ou restaure: $RUNPY -m pip install -r $SNAP"; exit 1; }
"$RUNPY" -c "import app; print('app: imports OK')" || {
  echo "[ERREUR] l'application ne s'importe plus."; exit 1; }
echo

# 5) nouvelles cles de preferences apparues dans l'exemple
if [ -f preferences.json ] && [ -f preferences.example.json ]; then
  "$RUNPY" -c "import json;a=json.load(open('preferences.json',encoding='utf-8'));b=json.load(open('preferences.example.json',encoding='utf-8'));n=[k for k in b if k not in a];print('Nouvelles cles de preferences disponibles: '+', '.join(n) if n else 'preferences.json a jour.')" 2>/dev/null
  echo "  (preferences.json n'est jamais ecrase: ajoute les cles voulues a la main)"
fi
echo
echo "=== Update OK ==="
[ -f CHANGELOG.md ] && echo "Nouveautes: voir CHANGELOG.md"
echo "Lance: ./run.sh  (ou ./boot_check.sh pour un diagnostic complet)"
